...
/// into SHM slots; the remote transport serializes it as bytes.
#[repr(C)]
#[derive(Clone, Copy, Debug)]
pub struct Envelope<T: Pod> {
    pub req_id: u64,
    pub payload: T,
}

// SAFETY: `Envelope<T>` is `#[repr(C)]`, contains only a `u64` and a
// `Pod` payload, and has no padding when `T`'s alignment is ≤ 8 (the
// common case for the types we accept; we round slot size up to 8
// bytes anyway).
unsafe impl<T: Pod> Pod for Envelope<T> {}
unsafe impl<T: Pod> Zeroable for Envelope<T> {}
